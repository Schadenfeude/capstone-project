...
public class ProductWidgetConfigureActivity extends AppCompatActivity {
    private static final String PREFS_NAME = "com.itrided.android.barracoda.widget.ProductWidget";
    ...
    public void onCreate(Bundle icicle) {
        ...
        recipeListViewModel.getProducts().observe(this, productEntries -> {
            final List<String> productList = new ArrayList<>();
            for (ProductEntry product : productEntries) {
                productList.add(product.getName());
            }
            adapter = new ArrayAdapter<>(this, R.layout.item_product_config_spinner, productList);
            ...
            final String storedValue = loadProductPreference(ProductWidgetConfigureActivity.this, appWidgetId);
            if (storedValue != null) {
        ...
        binding.recipeOkBtn.setOnClickListener(v -> {
            saveProductPreference(ProductWidgetConfigureActivity.this, appWidgetId,
    ...
    static void saveProductPreference(Context context, int appWidgetId, String text) {
        final SharedPreferences.Editor prefs = context.getSharedPreferences(PREFS_NAME, 0).edit();
        prefs.putString(PREF_PREFIX_KEY + appWidgetId, text);
        prefs.apply();
    }

    static String loadProductPreference(Context context, int appWidgetId) {
        final SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, 0);

        return prefs.getString(PREF_PREFIX_KEY + appWidgetId, null);
    }

    static void deleteProductPreference(Context context, int appWidgetId) {
